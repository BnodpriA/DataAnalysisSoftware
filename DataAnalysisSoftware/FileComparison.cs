...
namespace DataAnalysisSoftware
{
    public partial class FileComparison : Form
    {
            ...
            try
            {
                ...
                foreach (string file in files)
                {
                    if (i == 1)
                    {
                       ...
                       mainSummary(i, dgvHRMDataOfFirstFile);
                    }
                    else if (i == 2)
                    {
                ...
                for (int k = 0; k < dgvHRMDataOfFirstFile.Rows.Count; k++)
                {
                    ...
                    //int d1 = dgvHRMDataOfFirstFile.Rows[k].Cells[j].Value;
        ...
        private void mainSummary(int fileNumber, DataGridView dgvHRMDataOfFirstFile)
        {

            try
            {
                int grdCount = dgvHRMDataOfFirstFile.Rows.Count;
        ...
        private void dgvHRMDataOfFirstFile_MouseUp(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                int grdCount = dgvHRMDataOfFirstFile.SelectedRows.Count;
                ...
                int i = 0;
                foreach (DataGridViewRow row in dgvHRMDataOfFirstFile.SelectedRows)
                {
                    hr[i] = Convert.ToDouble(row.Cells[1].Value);
                    sp[i] = Convert.ToDouble(row.Cells[2].Value);
                    cd[i] = Convert.ToDouble(row.Cells[3].Value);
                ...
                ChunkOfData ck = new ChunkOfData(chunkDivision, hr, sp, cd, al, po);
                ck.Show();
            }
            catch (Exception ex)
            {
